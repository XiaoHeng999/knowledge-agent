...
- [x] **5.3.1** 实现全局错误边界 — React ErrorBoundary + Electron 未捕获异常处理
- [x] **5.3.2** 实现 TOP 5 错误 UI — API 失败（行内错误横幅）、模型不可用（断开状态）、导入失败（错误徽标）、数据库锁定（全局 Toast）、图谱渲染失败（列表视图降级）
- [x] **5.3.3** 实现错误恢复服务 — 重试逻辑（指数退避）、降级策略、错误上报
- [x] **5.3.4** 实现结构化日志服务 — `server/services/logger.ts`，JSON 格式 + 按操作成本追踪
- [x] **5.3.5** 实现调试模式 — 详细日志开关 + 性能指标面板
- **依赖**：P0.1（错误规格）, Phase 2-4
- **验证**：模拟各错误场景，UI 正确显示错误状态和恢复选项

### 5.4 骨架屏与加载状态

- [x] **5.4.1** 为 9 个主视图实现骨架屏 — 仪表盘、知识列表、时间线、研究仪表盘、对话、收件箱、领域概览、图谱、设置
- [x] **5.4.2** 实现闪烁阈值控制 — 100ms 以下不显示骨架屏
- [x] **5.4.3** 实现超时状态（> 10s）— 超时提示 + 重试按钮
- [x] **5.4.4** 实现 pi-mono 首次启动状态 — 初始化 2-5 秒的加载动画
- **依赖**：P0.10（骨架屏规格）, Phase 2-4
- **验证**：每个视图有正确的骨架屏 → 内容 → 错误状态过渡

### 5.5 空状态完善

- [x] **5.5.1** 实现 7 个空状态视图 — 空收件箱、空领域、空时间线、空研究仪表盘、空专家对话、无 API Key（全屏引导）、空知识列表
- [x] **5.5.2** 每个"无 API Key"空状态为全屏引导卡片（非常规空状态）
- [x] **5.5.3** 每个空状态包含：温暖感文案 + 主操作按钮 + 上下文说明
- **依赖**：P0.6（空状态设计）, Phase 2-4
- **验证**：每个视图在空数据时显示对应空状态

### 5.6 仪表盘优化

- [x] **5.6.1** 实现仪表盘焦点层级 — 首行 60/40 分割（今日概览占 60%，收件箱 40%）
- [x] **5.6.2** 实现领域理解分数可视化 — 仪表盘 + 领域概览中的进度环/渐变条
- [x] **5.6.3** 实现仪表盘统计卡片 — 主统计 48px/Bold + 次要统计
- **依赖**：P0.15（组件规格）, P0.16（仪表盘设计）
- **验证**：仪表盘有明确视觉焦点，领域理解分数可视化

### 5.7 Utility Process 集成

- [x] **5.7.1** 实现 Worker 进程启动 — Electron Utility Process 创建 + 通信通道
- [x] **5.7.2** 实现向量索引 Worker — 将 embedding 生成和索引移入 Worker
- [x] **5.7.3** 实现图谱计算 Worker — 力导向布局计算移入 Worker
- [x] **5.7.4** 实现 PDF 解析 Worker — PDF 文本抽取移入 Worker
- [x] **5.7.5** 实现 Worker 任务队列 — 优先级调度 + 进度报告 + 取消机制
- **依赖**：P0.3（Worker 架构规格）, Phase 3-4
- **验证**：CPU 密集操作不阻塞 UI 线程

### 5.8 跨平台构建

- [x] **5.8.1** 配置 macOS 构建 — DMG (arm64 + x64) + hardenedRuntime + 代码签名
- [x] **5.8.2** 配置 Windows 构建 — NSIS 安装器 + Portable 版本 + x64
- [x] **5.8.3** 配置 Linux 构建 — AppImage + deb + rpm + x64
- [x] **5.8.4** 实现路径抽象层验证 — `FileSystemProvider` 在三平台正确工作
- [x] **5.8.5** 配置自动更新 — electron-updater + GitHub Releases
- [x] **5.8.6** 三平台构建测试 — CI/CD 矩阵构建 + 手动测试
- **依赖**：Phase 1-4 完成
- **验证**：三平台构建成功，安装后可正常启动

### 5.9 性能优化

- [x] **5.9.1** 大知识图谱虚拟化渲染 — > 1000 节点虚拟化 + WebGL 降级
- [x] **5.9.2** 搜索性能优化 — 向量索引缓存 + 查询优化
- [x] **5.9.3** Electron 打包体积优化 — Tree-shaking + 按需加载 + ASAR 压缩
- [x] **5.9.4** 首屏加载优化 — 骨架屏 + 代码分割 + 预加载
- **依赖**：Phase 3-4
- **验证**：打包体积 < 200MB，首屏 < 3s，大图谱不卡顿

### 5.10 最终质量保障

- [x] **5.10.1** 端到端流程测试 — 首次启动 → 引导 → 创建领域 → 对话 → 研究 → 查看知识图谱
- [x] **5.10.2** 无障碍审计 — 键盘全流程导航、屏幕阅读器测试、对比度验证
- [x] **5.10.3** 4 种风格全流程测试 — 每种风格下全部功能正常
- [x] **5.10.4** 错误场景测试 — 模拟 API 失败、网络断开、数据库锁定、磁盘满
- [x] **5.10.5** 用户文档 — README + 快速开始 + 功能说明 + FAQ
- [x] **5.10.6** 开源准备 — LICENSE、CONTRIBUTING、GitHub Release 构建
- **依赖**：Phase 1-4 全部完成
